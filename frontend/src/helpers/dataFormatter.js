...
export default {
  ...
  customer_profilesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  customer_profilesOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  customer_profilesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  customer_profilesOneListFormatterEdit(val) {
    if (!val) return '';
  ...
  supplier_profilesManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.company_name);
  },
  supplier_profilesOneListFormatter(val) {
    if (!val) return '';
    return val.company_name;
  },
  supplier_profilesManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.company_name };
    });
  },
  supplier_profilesOneListFormatterEdit(val) {
    if (!val) return '';
