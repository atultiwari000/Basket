...
import { loadColumns } from './configureMarketing_campaignsCols';
...
const TableSampleMarketing_campaigns = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: marketing_campaignsNotify,
    refetch,
  } = useAppSelector((state) => state.marketing_campaigns);
  ...
  useEffect(() => {
    if (marketing_campaignsNotify.showNotification) {
      notify(
        marketing_campaignsNotify.typeNotification,
        marketing_campaignsNotify.textNotification,
      );
    }
  }, [marketing_campaignsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/marketing_campaigns/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/marketing_campaigns/marketing_campaigns-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/marketing_campaigns/marketing_campaigns-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
...
export default TableSampleMarketing_campaigns;
