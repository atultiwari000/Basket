...
import { loadColumns } from './configureSupplier_profilesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListSupplier_profiles from './ListSupplier_profiles';

const perPage = 10;

const TableSampleSupplier_profiles = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: supplier_profilesNotify,
    refetch,
  } = useAppSelector((state) => state.supplier_profiles);
  ...
  useEffect(() => {
    if (supplier_profilesNotify.showNotification) {
      notify(
        supplier_profilesNotify.typeNotification,
        supplier_profilesNotify.textNotification,
      );
    }
  }, [supplier_profilesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/supplier_profiles/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/supplier_profiles/supplier_profiles-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {supplier_profiles && Array.isArray(supplier_profiles) && !showGrid && (
        <ListSupplier_profiles
...
export default TableSampleSupplier_profiles;
