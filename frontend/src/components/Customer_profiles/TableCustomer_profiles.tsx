...
import { loadColumns } from './configureCustomer_profilesCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListCustomer_profiles from './ListCustomer_profiles';

const perPage = 10;

const TableSampleCustomer_profiles = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: customer_profilesNotify,
    refetch,
  } = useAppSelector((state) => state.customer_profiles);
  ...
  useEffect(() => {
    if (customer_profilesNotify.showNotification) {
      notify(
        customer_profilesNotify.typeNotification,
        customer_profilesNotify.textNotification,
      );
    }
  }, [customer_profilesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/customer_profiles/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/customer_profiles/customer_profiles-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {customer_profiles && Array.isArray(customer_profiles) && !showGrid && (
        <ListCustomer_profiles
...
export default TableSampleCustomer_profiles;
